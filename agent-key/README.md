...
GitHub Releases に `JEWEL1k-agent-key-vX.Y.Z-...` というファイルがある場合は、
Rust でビルドせずに使えます。配布物には以下が入っています。

- `agent-key`: CLI。Claude Code / Codex のフックやシェルから呼ぶ
- `agent-key-tray`: tray app。localhost API とデバイス接続を担当する
- `README-agent-key.md`: この説明のコピー
...
# Apple Silicon では arm64、Intel Mac では x86_64 の配布物を使う
./agent-key-tray
```

CLI をフックから呼べるように PATH の通った場所へ置きます。

```sh
mkdir -p "$HOME/.local/bin"
cp agent-key "$HOME/.local/bin/"
cp agent-key-tray "$HOME/.local/bin/"
...
未署名バイナリなので macOS に止められる場合があります。その場合は Finder で
右クリック → 開く、または以下を実行します。

```sh
xattr -dr com.apple.quarantine agent-key agent-key-tray
```
...
Windows では未署名のため SmartScreen 警告が出る場合があります。
...
trust してください。
hook が動かない場合は、`agent-key` が hook 実行環境の PATH に入っていない可能性が
あります。その場合は `.codex/hooks.json` の `command` を
`/Users/.../.local/bin/agent-key status ...` のような絶対パスに置き換えてください。
