...
void via_process(void) {
  ...
  // agent-key host packet piggybacking on the via raw-HID interface.
  // 0xA1 does not collide with any via command id (0x01..0x12, 0xFF).
  if (Ep2Buffer[0] == 0xA1) {
    if ((uint8_t)(Ep2Buffer[0] ^ Ep2Buffer[1] ^ Ep2Buffer[2] ^ Ep2Buffer[3] ^
                  Ep2Buffer[4]) == Ep2Buffer[5]) {
      agentkey_handle_host_packet(Ep2Buffer[1], Ep2Buffer[2], Ep2Buffer[3],
                                  Ep2Buffer[4]);
    }
    return; // no response report; corrupt frames are dropped silently
  }
