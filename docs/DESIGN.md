...
- 物理キーで承認操作を行う
  - 通常時: 単押しはHIDキーとして即時有効(複合デバイス版)。承認には使わない
  - 承認待ち中: HIDキー入力は抑止(複合デバイス版ファームウェアが `ST_APPROVAL` 中の
    key down を送らない)
  - ダブル押し: 承認(承認待ちがない時は詳細表示)
  - 長押し(>= 800ms): 拒否
...
2. リスク別ルール (`risk_policy.rs`):
   - none/low/medium/high: **ダブル押しで承認**、長押しで拒否
     (単押しは承認に使われない。通常時のHIDキー入力用)
