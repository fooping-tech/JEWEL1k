...
SerialTransport / HidTransport(trait only) / MockTransport
  ↓ (USB CDC 115200bps, A1/B1 バイナリパケット)
...
│   │       └── transport/            # Transport trait, Mock, Serial(feature),
│   │                                 #   HidTransport(traitのみ)
...
└── plugins/jewel1k-plugin-agent-key/ # Tauri v2 plugin本体
    ...
    │   ├── manager.rs                # 状態管理・イベントemit・LED送出
...
ファームウェアは `src/agentkey/agentkey.ino`(既存 `src/1key/1key.ino` ベース)。
...
- **HidTransport**: trait 定義のみ実装済み(`transport::HidTransport`)。
  QMK raw-HID 互換(usage page 0xFF60)の複合デバイス化により、
  キーボード機能と agent-key を同居させる。
- mobile (Swift/Kotlin) 対応は後回し。plugin は desktop のみを想定。
- tray app 本体はこのリポジトリには含めない(docs/TAURI_PLUGIN.md に組み込み手順)。
