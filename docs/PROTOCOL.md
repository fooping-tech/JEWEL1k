...
| 値   | イベント  | 意味                                             |
|------|-----------|--------------------------------------------------|
| 0x01 | single    | 単押し (<800ms、350msのダブル判定後に確定) = 通常時はHIDキー入力。承認には使われない |
| 0x02 | double    | ダブル押し (350ms以内に2回) = 承認 (承認待ちがない時は詳細表示) |
...
- 通常時: 単押しはHIDキーとして即時有効(複合デバイス版)。承認には一切使われない。
- 承認待ち中: `double` = 承認、`long` = 拒否、`very_long` = 緊急停止。
  riskレベル(none/low/medium/high)によらず承認操作は `double` に統一。
  critical はボタン操作に関係なく自動拒否。
- 複合デバイス版ファームウェア(`agentkey_hid.ino`)は、承認待ち中
  (`ST_APPROVAL`)のHIDキーボード入力を抑止する(B1 イベント送信は継続)。
