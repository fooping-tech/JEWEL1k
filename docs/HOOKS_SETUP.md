...
> `agent-key: command not found` の場合は `~/.cargo/bin`(Windows は
> `%USERPROFILE%\.cargo\bin`)が PATH に入っているか確認する。
...
| `PreToolUse` (Bash) | `agent-key hook pre-tool --risk high --json` | Bash 実行前に**物理キーで承認**(ダブル押し) |
...
> **auto モードでは物理承認をスキップ**: hook JSON の `permission_mode` が
> `auto` / `dontAsk` / `bypassPermissions`(auto-accept 系モード)の場合、
> `hook pre-tool` は承認要求を出さずに即 `allow` を返す(LED は黄=tool_running)。
> auto モードでも物理キーを必須にしたいなら `--always-gate` を付ける。
> `--risk critical` だけは auto モードでもゲートされ、常に自動拒否される。
...
| ファイル編集 | medium | ダブル押し |
| Bash / git push / デプロイ | high | ダブル押し |
| rm -rf / force push to main | critical | 常に自動拒否 |

### Codex

Codex にはブロッキングフックがないが `notify` がある。`~/.codex/config.toml` に:
...
- 実機なし(mock 接続時): 別ターミナルで `agent-key simulate double`
  → LED ログが承認を示し、Claude はツールを続行する
- 実機あり: JEWEL1k が赤く速点滅する。**ダブル押し**で承認、長押しで拒否
  (単押しでは承認されない)
